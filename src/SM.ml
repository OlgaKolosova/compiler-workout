...
*)                         
let rec eval env (cst, sta, (s, inp, out)) instruction = match instruction with
| [] -> (cst, sta, (s, inp, out))
| BINOP op :: p -> let y :: x :: st1 = sta in 
	let res = Expr.eval s (Binop (op, Const x, Const y))
	in eval env (cst, res :: st1, (s, inp, out)) p
...
| BEGIN (a, l) :: p -> let state = Language.State.push_scope s (a @ l) in
	let s, sta = List.fold_left (fun (s, x::stack) name -> (State.update name x s, stack)) (state, sta) a in eval env (cst, sta, (s, inp, out)) p
| CALL f :: p -> eval env ((p, s)::cst, sta, (s, inp, out)) (env#labeled f)
| END :: _ -> match cst with
    | (p, old_s)::cst -> eval env (cst, sta, (Language.State.drop_scope s old_s, inp, out)) p
    | _ -> (cst, sta, (s, inp, out))
...
List.concat (List.map (expr) (List.rev args)) @ [CALL f], false
 ...
 let rec compile_m p =
  let label = labelo#get "l_main" in
