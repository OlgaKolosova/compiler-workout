...
let labelGen = object
 ...
 let rec compileWithLabels p lastL =
  ...
  | Expr.Call (fName, argsE) -> let compile_args = List.flatten (List.map (expr) (List.rev argsE)) in
                                compile_args @ [CALL (fName, List.length argsE, true)]
  | Expr.Array xs         -> let compile_xs = List.flatten (List.map (expr) (List.rev xs)) in
                             compile_xs @ [CALL ("$array", (List.length compile_xs), true)]
  ...
  | Stmt.Assign (x, is, e) -> (match is with
                                 | [] -> (expr e @ [ST x]), false
                                 | _  -> let compile_id = List.fold_left (fun p id -> p @ (expr id)) [] (List.rev is) in
                                         compile_id @ expr e @ [STA (x, List.length is)], false
                              )
  | Stmt.Assign (x, [], e) -> (expr e @ [ST x]), false
  | Stmt.Assign (x, is, e) -> let compile_id = (List.fold_left (fun rest e -> expr e @ rest) [] (is @ [e])) in
                              (compile_id @ [STA(x, List.length is)]), false
  | Stmt.If (e, s1, s2) ->
  ...
  | Stmt.Skip -> [], false
  | Stmt.Call (fName, argsE) -> let compile_args = List.flatten (List.map (expr) (List.rev argsE)) in
                                compile_args @ [CALL (fName, List.length argsE, false)], false
...
let compile_d defs= let compile_d (name, (args,locals,body)) =
(let comp_body = compile_m body in
	[LABEL name; BEGIN (name, args, locals)] @ comp_body @ [END]) in List.flatten (List.map compile_d defs)
