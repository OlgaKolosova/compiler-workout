...
module Stmt =
 ...
 let rec eval cfg op = 
		...
		match op with
          ...
          | If (expr, thenStmt, elseStmt)        -> eval (st, input, output) (if Expr.int2bool (Expr.eval st expr) then thenStmt else elseStmt)
          | While (expr, wStmt)                  -> if Expr.int2bool (Expr.eval st expr) then eval (eval (st, input, output) wStmt) op else (st, input, output)
          | RepeatUntil (ruStmt, expr)           -> let (sNew, iNew, oNew) = eval (st, input, output) ruStmt in
													if not (Expr.int2bool (Expr.eval sNew expr)) then eval (sNew, iNew, oNew) op else (sNew, iNew, oNew)
